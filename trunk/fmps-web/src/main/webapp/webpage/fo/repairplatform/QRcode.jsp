...
<script type="text/javascript">
	function getNewQRcode() {
		...
		img.src = '${webRoot}/fo/repairFactoryController.do?getQRcode&QRphone='
				+ QRphone.value+'&uuid='+uuid+'&openid=${openid}';
	}
	//每20分钟刷新一次页面 
	function myrefresh() 
	{ 
		...
		var uuid='${uuid}';
		 $.ajax({
				...
				success: function(data){
					if(data=="1"){
						location.href ="${webRoot}/fo/repairFactoryController.do?method=toCaseList&openid=${openid}&uuid=${uuid}" 
					}
...
<body>
	<div class="page">
		<div id="wrapper" data-am-widget="list_news"
			...
			<div class="erweima_bg">
				<div class="erweima">
					<img id="QRcode"
						src="${webRoot}/fo/repairFactoryController.do?getQRcode&openid=${openid}&uuid=${uuid}"
 ...
 -->				<div class="erweima_tips">
					...
					<div class="erweima_tips_text_2">
						...
						如处理其他报案手机的闪赔案件，可点此 <a style="color: #0f90d2;"href="${webRoot}/fo/repairFactoryController.do?method=QRcodePagePhone&uuid=${uuid}&openid=${openid}">输入报案电话</a>
					</div>
				</div>
				<div class="erweima_di">&nbsp;</div>
					...
					<div class="erweima_tips_text_2">
						<div style="float: left; width: 70%;">
							<input id="QRphone" name="" type="text"
								style="height: 30px; width: 90%; border: 1px solid #dedede;">
						</div>
