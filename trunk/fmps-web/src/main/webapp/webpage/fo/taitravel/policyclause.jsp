...
<c:choose>
	<c:when test="${affiliatedList.size()>0}">	
		 ...
		 <div class="top_text">请认真阅读以下产品条款。</div>
		  ...
		  <c:forEach items="${affiliatedList}"	var="clause" varStatus="status">
		      <dl class="am-accordion-item">
					<dt class="am-accordion-title">
						<a href="${webRoot}/fo/taiwanController.do?method=clauseDetail&openid=${openid}&policyno=${policyno}&productcode=${productcode}&afid=${clause.id}"><div id="description">${clause.description}</div></a>
					</dt>
				</dl>
			</c:forEach>	
		  </section>		
