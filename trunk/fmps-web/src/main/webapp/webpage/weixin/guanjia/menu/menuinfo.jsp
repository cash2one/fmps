...
<html>
 ...
 <script type="text/javascript">
 	$(document).ready(function(){ 
 	   ...
 	   var fatherIdTemp = "${fatherId}";
 	   if(true){
 	   		var supMenuIdObj = $("#fatherName");
 	   		var supMenuId="${menuEntity.id}";
 	    	var supMenuname="${fatherName}";
	 		supMenuIdObj.html("");
	 		$.ajax({
	 			...
	 			success:function(data){
	 				var msg="";
	 				for(var i=0;i<data.length;i++){
	 					if(data[i].id==fatherIdTemp){
	 						msg += "<option value='"+data[i].id+"' selected='selected'>"+data[i].name+"</option>";
	 					}else{
	 						msg += "<option value='"+data[i].id+"'>"+data[i].name+"</option>";
	 					}
	 				}
	 				if(supMenuname==''||supMenuname==null){
	 					msg += "<option value='firstLevelMenu' selected='selected'> </option>";
	 				}else{
	 					msg += "<option value='firstLevelMenu'> </option>";
	 				}
  ...
  <c:if test="${accountid!=null}">
   ...
   <table style="width:100%" cellpadding="0" cellspacing="1" class="formtable">
    ...
    <tr>
     ...
     <td class="value" colspan="3">
      <%--      
      <input id="url" class="inputxt" name="status" value="${status}" datatype="*" nullmsg="请填写菜单状态">
      <span class="Validform_checktip">状态（1、有效 2、暂停）</span>
      --%>
      <select name="status" id="status">
      	<option value="1"  <c:if test="${status=='1'}">selected="selected"</c:if>>有效</option>
      	<option value="2"  <c:if test="${status=='2'}">selected="selected"</c:if>>暂停</option>
      </select>
