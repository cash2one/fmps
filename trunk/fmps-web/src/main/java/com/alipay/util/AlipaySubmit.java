...
public class AlipaySubmit {
	...
	public static String buildRequestMysign(Map<String, String> sPara,
		...
		String charset = configMap.get("charset").toString();
		if (sec_id.equals("MD5")) {
			mysign = MD5.sign(prestr, key, charset);
		}
		if (sec_id.equals("0001")) {
			String private_key = configMap.get("private_key").toString();
			mysign = RSA.sign(prestr, private_key, charset);
		}
		return mysign;
	}
	...
	private static Map<String, String> buildRequestPara(
			Map<String, String> sParaTemp, Map<String,String> configMap) {
		// 除去数组中的空值和签名参数
		...
		if (!sPara.get("service").equals("alipay.wap.trade.create.direct")
			...
			sPara.put("sign_type", configMap.get("sec_id").toString());
		}
	...
	public static String buildRequest(String ALIPAY_GATEWAY_NEW,
			...
			String strButtonName, Map<String,String> configMap) {
		// 待请求参数数组
		Map<String, String> sPara = buildRequestPara(sParaTemp, configMap);
		...
		sbHtml.append("<form id=\"alipaysubmit\" name=\"alipaysubmit\" action=\""
				+ ALIPAY_GATEWAY_NEW
				+ "_input_charset="
				+ configMap.get("charset").toString()
				+ "\" method=\""
				+ strMethod + "\">");
	...
	public static String buildRequest(String ALIPAY_GATEWAY_NEW,
			Map<String, String> sParaTemp, String strMethod,
			String strButtonName, String strParaFileName, Map<String,String> configMap) {
		// 待请求参数数组
		Map<String, String> sPara = buildRequestPara(sParaTemp, configMap);
		List<String> keys = new ArrayList<String>(sPara.keySet());
	...
	public static String buildRequest(String ALIPAY_GATEWAY_NEW,
			...
			Map<String, String> sParaTemp, Map<String,String> configMap) throws Exception {
		// 待请求参数数组
		Map<String, String> sPara = buildRequestPara(sParaTemp, configMap);
		...
		// 设置编码集
		request.setCharset(configMap.get("charset").toString());
	 ...
	 * @throws Exception
	 */
	public static String getRequestToken(String text, Map<String,String> configMap)
			throws Exception {
		...
		if (paraText.get("res_data") != null) {
			...
			String sec_id = configMap.get("sec_id").toString();
			if (sec_id.equals("0001")) {
				res_data = RSA.decrypt(res_data, configMap.get("private_key")
						.toString(), configMap.get("charset").toString());
