...
public class WeixinAccountServiceImpl extends CommonServiceImpl implements
	...
	/*
	 * 根据公众号原始ID获取ACCESS_TOKEN
	 * 
	 * (non-Javadoc)
	 * 
	 * @see
	 * weixin.guanjia.account.service.WeixinAccountServiceI#getAccessToken(java
	 * .lang.String)
	 */
	public String getAccessToken(String accountId) {

		WeixinAccountEntity weixinAccountEntity = this.findUniqueByProperty(
				WeixinAccountEntity.class, "weixin_accountid", accountId);
		return this.getAccessToken(weixinAccountEntity);
	}
	...
	public String getAccessTokenFromAccountEntity() {
		List<WeixinAccountEntity> weixinAccountEntityList = this.findValidWeixinAccounts();
		WeixinAccountEntity weixinAccountEntity = weixinAccountEntityList.get(0);
		return this.getAccessToken(weixinAccountEntity);
	}

	/**
	 * 根据微信账户实体类获取ACCESS_TOKEN
	 * 
	...
	public String getAccessToken(WeixinAccountEntity weixinAccountEntity) {
		...
		if (token != null && !"".equals(token)) {
			// 判断有效时间 是否超过2小时
			java.util.Date end = new java.util.Date();
			java.util.Date start = new java.util.Date(weixinAccountEntity
					.getAddtoekntime().getTime());
			if ((end.getTime() - start.getTime()) / 1000 / 3600 >= 2) {
				// 失效 重新获取
				String requestUrl = WeixinUtil.access_token_url.replace(
						"APPID", weixinAccountEntity.getAccountappid())
						.replace("APPSECRET",
	...
	public List<WeixinAccountEntity> findValidWeixinAccounts() {
		return this.findByProperty(WeixinAccountEntity.class, "status",
