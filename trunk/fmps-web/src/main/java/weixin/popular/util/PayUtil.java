...
public class PayUtil {
	...
	/**
	 * 生成支付JS请求JSON
	 * @param package_   订单详情扩展字符串, 统一支付接口返回的prepay_id 参数值
	 * @param appId      公众号id
	 * @param PartnerKey  商户支付密钥
	 * @return
	 */

	public static String generatePayJsRequestJson(String package_,String appId,String PartnerKey){
		PayJsRequest payJsRequest = new PayJsRequest();
		payJsRequest.setAppId(appId);
		payJsRequest.setTimeStamp(System.currentTimeMillis()/1000+"");
		payJsRequest.setNonceStr(UUID.randomUUID().toString());
		...
		String paySign = SignatureUtil.generateSign(mapS,PartnerKey);
		payJsRequest.setPaySign(paySign);
		return JsonUtil.toJSONString(payJsRequest);
	}
