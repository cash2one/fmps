...
public class MediaAPI extends BaseAPI {
	...
	/**
	 * 上传媒体文件 媒体文件在后台保存时间为3天，即3天后media_id失效。
	 * 
	 * @param access_token
	 * @param mediaType
	 * @param media 多媒体文件有格式和大小限制，如下： 图片（image）: 128K，支持JPG格式
	 *            语音（voice）：256K，播放长度不超过60s，支持AMR\MP3格式 视频（video）：1MB，支持MP4格式
	 *            缩略图（thumb）：64KB，支持JPG格式
	 * @return
	 */
	public Media mediaUpload(String access_token, MediaType mediaType,
			File media) {
		...
		HttpEntity reqEntity = MultipartEntityBuilder.create()
				...
				.addTextBody("type", mediaType.uploadType()).build();
		httpPost.setEntity(reqEntity);
		return localHttpClient.execute(httpPost,
	 ...
	 * @param access_token
	 * @param mediaType
	 * @param inputStream 多媒体文件有格式和大小限制，如下： 图片（image）: 128K，支持JPG格式
	 *            语音（voice）：256K，播放长度不超过60s，支持AMR\MP3格式 视频（video）：1MB，支持MP4格式
	 *            缩略图（thumb）：64KB，支持JPG格式
	 * @return
	 */
	public Media mediaUpload(String access_token, MediaType mediaType,
		...
		InputStreamBody inputStreamBody = new InputStreamBody(inputStream,
				mediaType.mimeType(), "temp." + mediaType.fileSuffix());
		HttpEntity reqEntity = MultipartEntityBuilder.create()
	 ...
	 * @param access_token
	 * @param mediaType
	 * @param uri 多媒体文件有格式和大小限制，如下： 图片（image）: 128K，支持JPG格式
	 *            语音（voice）：256K，播放长度不超过60s，支持AMR\MP3格式 视频（video）：1MB，支持MP4格式
	 *            缩略图（thumb）：64KB，支持JPG格式
	 * @return
	 */
	public Media mediaUpload(String access_token, MediaType mediaType, URI uri) {
		...
		try {
			HttpEntity entity = tempHttpClient.execute(
					RequestBuilder.get().setUri(uri).build()).getEntity();
			HttpEntity reqEntity = MultipartEntityBuilder
					.create()
					.addBinaryBody("media", EntityUtils.toByteArray(entity),
							ContentType.get(entity),
							"temp." + mediaType.fileSuffix())
					.addTextBody("access_token", access_token)
					.addTextBody("type", mediaType.uploadType()).build();
			httpPost.setEntity(reqEntity);
			return localHttpClient.execute(httpPost,
	...
	public byte[] mediaGet(String access_token, String media_id) {
		...
		HttpResponse httpResponse = mediaGetResponse(access_token, media_id);

		try {
			if (httpResponse != null)
				return EntityUtils.toByteArray(httpResponse.getEntity());
		} catch (IOException e) {
			logger.error("convert http response to byte failure,cause by:"
					+ e.getMessage());
			e.printStackTrace();
		}
	...
	public HttpResponse mediaGetResponse(String access_token, String media_id) {
		HttpUriRequest httpUriRequest = RequestBuilder.get()
				.setUri(MEDIA_URI + "/cgi-bin/media/get")
				.addParameter("access_token", access_token)
				.addParameter("media_id", media_id).build();
		logger.info("get media URL====>" + httpUriRequest.getURI());
