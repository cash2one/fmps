...
public class ListtoMenu {
			...
			for (TSFunction function : functions) {
				...
				if (function.getTSFunction().getId().equals(pFunction.getId())) {
					...
					if (function.getFunctionName().equals(
							Constants.WEIXIN_USER_FUNCTION_WECHAT)
							|| function
									.getFunctionName()
									.equals(Constants.WEIXIN_USER_FUNCTION_WECHAT_HISTORY)) {// patrick.z20150303
						menuString
								.append("<li><div title=\""
										...
										+ "\"> <a class=\""
										+ function.getFunctionName()
										+ "\" href=\""+function.getFunctionUrl()+"\" target=\"_blank\" > <span class=\"icon "
										+ icon
										+ "\" >&nbsp;</span> <span class=\"nav\" >"
										+ function.getFunctionName()
										+ "</span></a></div></li>");
						...
						menuString.append("<li><div onclick=\"addTab(\'"
								...
								+ function.getFunctionUrl() + "\" iconCls=\""
								+ icon + "\"> <a class=\""
								+ function.getFunctionName()
	...
	public static String getEasyuiMultistageTree(
			Map<Integer, List<TSFunction>> map, String style) {
		...
		if ("easyui".equals(style)) {
			for (TSFunction function : list) {
				...
				menuString.append("<span>").append(function.getFunctionName())
						.append("</span>");
				int submenusize = function.getTSFunctions().size();
				if (submenusize == 0) {
					menuString.append("</li>");
			...
			for (TSFunction function : list) {
				menuString.append("<div   title=\""
						...
						+ function.getTSIcon().getIconClas() + "\">");
				int submenusize = function.getTSFunctions().size();
				if (submenusize == 0) {
					menuString.append("</div>");
				}
				if (submenusize > 0) {
					menuString
							.append("<ul class=\"easyui-tree tree-lines\"  fit=\"false\" border=\"false\">");
				}
				menuString.append(getChildOfTree(function, 1, map));
				if (submenusize > 0) {
					menuString.append("</ul></div>");
	 ...
	 * @return
	 */
	private static String getChild(TSFunction parent, int level,
			Map<Integer, List<TSFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		...
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())) {
				if (function.getTSFunctions().size() == 0
					...
					menuString
							.append("<div  class=\"easyui-accordion\"  fit=\"false\" border=\"false\">");
					menuString.append("<div></div>");// easy ui
														// 默认展开第一级,所以这里设置一个控制,就不展开了
					menuString.append("<div title=\""
							+ function.getFunctionName() + "\" iconCls=\""
							+ function.getTSIcon().getIconClas() + "\"><ul>");
	 ...
	 * @return
	 */
	private static String getChildOfTree(TSFunction parent, int level,
			Map<Integer, List<TSFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(level);
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())) {
				if (function.getTSFunctions().size() == 0
					...
					menuString.append("<li state=\"closed\" iconCls=\""
							+ function.getTSIcon().getIconClas() + "\" ><span>"
							+ function.getFunctionName() + "</span>");
					menuString.append("<ul >");
	...
	private static String getLeaf(TSFunction function) {
		...
		if (function.getFunctionName().equals(
				Constants.WEIXIN_USER_FUNCTION_WECHAT) ||
				function.getFunctionName().equals(Constants.WEIXIN_USER_FUNCTION_WECHAT_HISTORY)) {// patrick.z 20150303
			...
			menuString.append(function.getFunctionName());
			menuString
					.append("\" href=\""+function.getFunctionUrl()+"\" target=\"_blank\" > <span class=\"icon ");
			menuString.append(icon);
	...
	private static String getLeafOfTree(TSFunction function) {
		...
		if (function.getFunctionName().equals(
			...
			menuString.append(icon);
			menuString
					.append("\"> <a href=\""+function.getFunctionUrl()+"\" target=\"_blank\" >");
	...
	public static String getBootstrapMenu(Map<Integer, List<TSFunction>> map) {
		...
		for (TSFunction pFunction : pFunctions) {
			// 是否有子菜单
			boolean hasSub = pFunction.getTSFunctions().size() == 0 ? false
					: true;

			// 绘制一级菜单
			menuString.append("	<li class=\"dropdown\"> ");
			menuString
					.append("		<a href=\"javascript:;\" class=\"dropdown-toggle\" data-toggle=\"dropdown\"> ");
			menuString
					.append("			<span class=\"bootstrap-icon\" style=\"background-image: url('"
							+ pFunction.getTSIcon().getIconPath()
							+ "')\"></span> "
	 ...
	 * @throws
	 */
	private static String getBootStrapChild(TSFunction parent, int level,
			Map<Integer, List<TSFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		List<TSFunction> list = map.get(level);
		...
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())) {
				boolean hasSub = function.getTSFunctions().size() != 0
						&& map.containsKey(level + 1);
				String menu_url = function.getFunctionUrl();
				if (StringUtils.isNotEmpty(menu_url)) {
					menu_url += "&clickFunctionId=" + function.getId();
				}
				menuString.append("		<li onclick=\"showContent(\'"
						+ function.getFunctionName() + "\',\'" + menu_url
				...
				menuString
						.append("				<span class=\"bootstrap-icon\" style=\"background-image: url('"
								+ function.getTSIcon().getIconPath()
								+ "')\"></span>		 ");
				menuString.append(function.getFunctionName());
				menuString.append("			</a> ");
				if (hasSub) {
					menuString.append(getBootStrapChild(function, level + 1,
							map));
				}
				menuString.append("		</li> ");
			}
	...
	public static String getWebosMenu(Map<Integer, List<TSFunction>> map) {
		...
		for (TSFunction pFunction : pFunctions) {
			...
			menuString.append("{\"id\":\"" + pFunction.getId()
					+ "\",\"name\":\"" + pFunction.getFunctionName()
					+ "\",\"path\":\"" + pFunction.getTSIcon().getIconPath()
			...
			if (hasSub) {
				...
				DeskpanelString
						.append(getWebosDeskpanelChild(pFunction, 1, map));
		...
		desk = DeskpanelString.substring(0,
				DeskpanelString.toString().length() - 1);
		desk += "}";
	 ...
	 * @throws
	 */
	private static String getWebosChild(TSFunction parent, int level,
			Map<Integer, List<TSFunction>> map) {
		StringBuffer menuString = new StringBuffer();
		String menu = "";
		List<TSFunction> list = map.get(level);
		...
		for (TSFunction function : list) {
			if (function.getTSFunction().getId().equals(parent.getId())) {
				...
				// }
				menuString.append("\"" + function.getId() + "\":");
				menuString.append("{\"id\":\"" + function.getId()
						+ "\",\"name\":\"" + function.getFunctionName()
						+ "\",\"path\":\"" + function.getTSIcon().getIconPath()
						+ "\",\"url\":\"" + function.getFunctionUrl()
						+ "\",\"level\":\"" + function.getFunctionLevel()
						+ "\"}");
	...
	private static String getWebosDeskpanelChild(TSFunction parent, int level,
			Map<Integer, List<TSFunction>> map) {
		StringBuffer DeskpanelString = new StringBuffer();
		String desk = "";
		List<TSFunction> list = map.get(level);
	...
	private static String getWebosDataChild(TSFunction parent, int level,
			Map<Integer, List<TSFunction>> map) {
		StringBuffer dataString = new StringBuffer();
		String data = "";
		List<TSFunction> list = map.get(level);
	...
	private static String getIconAndNameForDesk(TSFunction function) {
		...
		String colName = function.getTSIconDesk() == null ? null : function
				.getTSIconDesk().getIconPath();
		colName = (colName == null || colName.equals("")) ? "plug-in/sliding/icon/default.png"
				: colName;
