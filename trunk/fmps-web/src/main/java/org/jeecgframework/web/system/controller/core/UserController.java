...
public class UserController {
	...
	public ModelAndView changepasswordforuser(TSUser user,
			HttpServletRequest req) {
		if (StringUtil.isNotEmpty(user.getId())) {
			user = systemService.getEntity(TSUser.class, user.getId());
			req.setAttribute("user", user);
			idandname(req, user);
			org.jeecgframework.core.util.LogUtil.info(user.getPassword()
					+ "-----" + user.getRealName());
		}
	...
	public void datagrid(TSUser user, HttpServletRequest request,
		...
		Short[] userstate = new Short[] { Globals.User_Normal,
				Globals.User_ADMIN, Globals.User_Forbidden };
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			if (users != null) {
				...
				if (shopSymbol != null && "shop".equals(shopSymbol)) {
					TSRole tsRole = this.systemService.findUniqueByProperty(
							TSRole.class, "roleCode", "manager");
					roleid = tsRole.getId();
				}
	...
	public void testSearch(TSUser user, HttpServletRequest request,
			HttpServletResponse response, DataGrid dataGrid) {
		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);
	...
	public AjaxJson saveStyle(HttpServletRequest request,
		...
		if (user != null) {
			...
			Client client = (Client) request.getSession().getAttribute(
					Constants.SESSION_KEY_BO_USER);
			client.getFunctions().clear();
		} else {
			j.setMsg("请登录后再操作");
		}
		return j;
	 ...
	 * @return
	 */
	@RequestMapping(params = "memcachedindex")
	public String memcachedindex() {
		return "system/function/memcached";
	}
	 ...
	 * @return
	 */
	@RequestMapping(params = "getMemcachedValue")
	public String getMemcachedValue(HttpServletRequest request) {
		String memcachedkey = request.getParameter("memcachedkey");
		String openid = request.getParameter("openidkey");
		String registNokey = request.getParameter("registNokey");
		String toJspValues = "";
		if (!StringUtil.isEmpty(memcachedkey)) {
			toJspValues = toJspValues + "通过memcachedkey查询到的值为:"
					+ CachedUtils.get(memcachedkey) + "";
		} else if (!StringUtil.isEmpty(openid)) {
			toJspValues = toJspValues + "通过openid查询到的微信闪赔状态为:"
					+ this.getReportStatus(openid);

		} else if (!StringUtil.isEmpty(registNokey)) {
			List<ReportInfo> reportInfoList = reportInfoService.findByProperty(
					ReportInfo.class, "registNo", registNokey);
			if (reportInfoList.size() > 0) {
				openid = reportInfoList.get(0).getOpenid();
				toJspValues = toJspValues + "通过registNokey查询到的微信闪赔状态为:"
						+ this.getReportStatus(openid);
			} else {
				toJspValues = toJspValues + "通过registNokey没有查询到微信闪赔案件";
			}
		}
		request.setAttribute("memcachedvalue", toJspValues);
		return "system/function/memcached";
	}

	private String getReportStatus(String openid) {
		Progress claimsProgress = null;
		try {
			WeChatContext weChatContext = (WeChatContext) CachedUtils
					.get(Constants.MEMKEY_WEIXIN_WeChat_claims + openid);
			claimsProgress = (Progress) weChatContext
					.get(Constants.WEIXINCLAIMS_claimsProgress);

		} catch (Exception e) {
			return "memcached查询出错";
		}
		return claimsProgress.toString();

	}
	
	@RequestMapping(params = "modifyMemcached")
	@ResponseBody
	public AjaxJson modifyMemcached(HttpServletRequest request,
			String memcachedKey,String cachedValue,int modifyType) {
		...
		//SET
		if(modifyType == Constants.MEMCACHED_MODIFY_TYPE_SET){
			CachedUtils.set(memcachedKey,cachedValue);
			returnParams.put("resultValue", "操作成功");
			message +="进行memcached的SET操作，"+"将key: "+memcachedKey+",设置为："+cachedValue;
			systemService.addLog(message, Globals.Log_Type_UPDATE,
					Globals.Log_Leavel_INFO);
		//DELETE
		}else if(modifyType == Constants.MEMCACHED_MODIFY_TYPE_DELETE){
			CachedUtils.delete(memcachedKey);
			returnParams.put("resultValue", "操作成功");
			message +="进行memcached的DELETE操作，"+"将key: "+memcachedKey+" 删除。";
			systemService.addLog(message, Globals.Log_Type_DEL,
					Globals.Log_Leavel_INFO);
		//DECR
		}else if(modifyType == Constants.MEMCACHED_MODIFY_TYPE_DECR){
			CachedUtils.desc(memcachedKey, Integer.parseInt(cachedValue));
			returnParams.put("resultValue", "操作成功");
			message +="进行memcached的DECR操作，"+"将key: "+memcachedKey+",减少："+cachedValue;
			systemService.addLog(message, Globals.Log_Type_UPDATE,
					Globals.Log_Leavel_INFO);
		//INCR
		}else if(modifyType == Constants.MEMCACHED_MODIFY_TYPE_INCR){
			returnParams.put("resultValue", CachedUtils.incr(memcachedKey, 0));
			message +="进行memcached的INCR操作，"+"将key: "+memcachedKey+",增加："+cachedValue;
			systemService.addLog(message, Globals.Log_Type_UPDATE,
					Globals.Log_Leavel_INFO);
		//GET
		}else if(modifyType == Constants.MEMCACHED_MODIFY_TYPE_GET){
			returnParams.put("resultValue", CachedUtils.get(memcachedKey));
			message +="进行memcached的GET操作，"+"获取key: "+memcachedKey;
			systemService.addLog(message, Globals.Log_Type_OTHER,
					Globals.Log_Leavel_INFO);
		}
		
		returnJson.setAttributes(returnParams);
		return returnJson;
	}
