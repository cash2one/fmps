...
public class SystemServiceImpl extends CommonServiceImpl implements SystemService {
	...
	/**
	 * 添加日志(20160130 zhangyaoming)
	 * 
	 */
	public void addLogNotRequest(String logcontent, Short loglevel, Short operatetype) {
		try {
			TSLog log = new TSLog();
			log.setLogcontent(logcontent);
			log.setLoglevel(loglevel);
			log.setOperatetype(operatetype);
			log.setNote(oConvertUtils.getRealIp());
			log.setOperatetime(DateUtils.gettimestamp());
			commonDao.save(log);
		} catch (SocketException e) {
			logger.error("添加日志(非request)异常",e);
		}
	}
	...
	@Override
	public void saveOrUpdateWeiXinTemplateMessageSendLog(
			WeiXinTemplateMessageSendLog sendLog){
		WeiXinTemplateMessageRecord record = sendLog.getTemplateMessageRecord();
		record.setStatus(sendLog.getStatus());
		
		try {
			getSession().saveOrUpdate(sendLog);
			getSession().saveOrUpdate(record);
			getSession().flush();
			if (logger.isDebugEnabled()) {
				logger.debug("WeiXinTemplateMessageSendLog和WeiXinTemplateMessageRecord添加或更新成功");
			}
		} catch (RuntimeException e) {
			logger.error("WeiXinTemplateMessageSendLog和WeiXinTemplateMessageRecord添加或更新异常", e);
			throw e;
		}
