 ...
 * 
 */
package cn.com.fubon.common.controller;
...
//					graphics.setFont(new Font("Arial Black", Font.ITALIC, 18));
			...
			session.setAttribute(Constants.SESSION_KEY_RAND_CODE, resultCode);
			
			/*20150104 patrick.z*/
			//把验证码另存副本在memcached
			String openid = request.getParameter("randCode");
			String memberKey = "randCode"+openid;
			CachedUtils.set(memberKey,resultCode);
			logger.info("RandCode IN ServletSessioon ====> " + resultCode +", SessionID:"+session.getId());	
			logger.info("memcached IN ServletSessioon ====> " + CachedUtils.get("randCode"+openid));	
			// 图象生效
			graphics.dispose();
	 ...
	 * @return 随机码
	 */
	public static String exctractRandCode(String type,String length) {
		final String randCodeType = type;
        int randCodeLength = Integer.parseInt(length);
        if (randCodeType == null) {
			return RandCodeImageEnum.ALL_CHAR.generateStr(randCodeLength);
		} else {
			switch (randCodeType.charAt(0)) {
...
enum RandCodeImageEnum {
	 ...
	 * 混合字符串
	 */
	ALL_CHAR("123456789abcdefghjkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ"),
	/**
	 * 字符
	 */
