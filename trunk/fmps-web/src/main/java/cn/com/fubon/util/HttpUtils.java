...
import cn.com.fubon.entity.Attachment;
...
public class HttpUtils {
	 ...
	 */
	public static String postGBK(String url, Map<String, String> params)
			throws IOException, ExecutionException, InterruptedException {
		AsyncHttpClient http = new AsyncHttpClient();
		AsyncHttpClient.BoundRequestBuilder builder = http.preparePost(url);
		...
		Future<Response> f = builder.execute();
		String body = f.get().getResponseBody("gbk");
		http.close();
		return body;
	}
	 ...
	 * @throws IOException
	 */
	public static Attachment download(String url) throws ExecutionException,
		...
		if (f.get().getContentType().equalsIgnoreCase("text/plain")) {
			...
			BufferedInputStream bis = new BufferedInputStream(f.get()
